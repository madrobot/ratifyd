...
const ISSUER = 'peer-123'

const makePayload = (overrides?: Partial<Omit<JWTPayload, 'iat' | 'exp' | 'jti' | 'iss'>>) => ({
  room: 'test-room',
  role: ROLES.OWNER,
  ...overrides,
})
...
describe('mintJWT', () => {
  it('returns a ClaimToken with header, payload, signature, and raw', async () => {
    ...
    const token = await mintJWT(makePayload(), ISSUER, pair.privateKey)
    ...
    expect(token.payload.iss).toBe(ISSUER)
  ...
  it('sets exp based on expirySeconds', async () => {
    const pair = await generateSigningKeyPair()
    const token = await mintJWT(makePayload(), ISSUER, pair.privateKey, 3600)
  ...
  it('generates unique jti each time', async () => {
    const pair = await generateSigningKeyPair()
    const token1 = await mintJWT(makePayload(), ISSUER, pair.privateKey)
    const token2 = await mintJWT(makePayload(), ISSUER, pair.privateKey)
  ...
  it('preserves all roles correctly', async () => {
    ...
    for (const role of [ROLES.OWNER, ROLES.MODERATOR, ROLES.GUEST]) {
      const token = await mintJWT(makePayload({ role }), ISSUER, pair.privateKey)
...
describe('verifyJWT', () => {
  ...
  it('returns INVALID_SIGNATURE for wrong public key', async () => {
    ...
    const token = await mintJWT(makePayload(), ISSUER, pair1.privateKey)
  ...
  it('returns TOKEN_EXPIRED for expired token', async () => {
    const pair = await generateSigningKeyPair()
    const token = await mintJWT(makePayload(), ISSUER, pair.privateKey, 1)
    vi.useFakeTimers()
